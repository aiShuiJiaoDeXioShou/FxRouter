...
public class Router {
    ...
    public Router(String basePackagePath, Stage stage) {
        ...
        if (Objects.isNull(isRoute("/"))) throw new RuntimeException("必须要指定一个主路由！！！\n指定一个组件的路由地址为’/‘");
        // 默认路由路径为/为主路由
        toRoute("/");
    }
    ...
    private Router parseFxRouter() {
        ...
        for (Class<?> clazz : classSet) {
            ...
            route.setFxRoute(fxRoute)
                    ...
                    .setPaths(RouterUtil.paths(route.getPath()));
    ...
    private void toChildrenRouter(Route route,String path) {
        ...
        if (panes.size() > 0) {
            for (int i = 0; i < panes.size(); i++) {
                switchRouterNode(panes.get(i),routers.get(path).getNode());
            }
        }
    }
